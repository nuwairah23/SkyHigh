...
<div class="table-pad">
	...
	<c:forEach items="${bookings}" var="b" varStatus="bookings">
		<tr>
			...
			<td><a href="BookingController?action=viewBookingstaff&bookingid=<c:out value="${b.bookingid}"/>" class="sub">View</a></td>
			<td><input type="hidden" id="bookingid-${bookings.index}" value="<c:out value="${b.bookingid}"/>"><button class="sub" onclick="confirmation('${bookings.index}')">Delete</button></td>
		</tr>
	</c:forEach>
	</table>
</div>
